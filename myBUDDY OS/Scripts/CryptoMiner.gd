...
func _on_CryptoMiner_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton:
		if event.is_pressed():
			health -= stats.base_attack * stats.ai_multiplier
			soundPlayer.play()


func _on_EnemyHitSoundPlayer_finished():
	if health <= 0:
		stats.set_crypto_miners_repurposed(stats.crypto_miners_repurposed + 1)
