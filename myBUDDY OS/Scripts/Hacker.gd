...
func _on_Hacker_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton:
		if event.is_pressed():
			health -= stats.base_attack * stats.firewall_multiplier
			soundPlayer.play()
...
func _on_EnemyHitSoundPlayer_finished():
	if health <= 0:
		stats.set_hackers_stopped(stats.hackers_stopped + 1)
